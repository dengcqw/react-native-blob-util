...
public class UploadResumeDefaultController implements IUploadResumeController {
        ...
        if (mSharedPreferences != null) {
            try {
                ...
                if (TextUtils.isEmpty(vodSessionKey) || TextUtils.isEmpty(uploadId)) {
                    ...
                    mShareEditor.putString(md5Key, comment);
                }
                mShareEditor.commit();
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public ResumeCacheData getResumeData(String filePath) {
        ...
        String md5Key = TVCUtils.getFileMD5(filePath);
        if (mSharedPreferences.contains(md5Key)) {
            try {
                JSONObject json = new JSONObject(mSharedPreferences.getString(md5Key, ""));
